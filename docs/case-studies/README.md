...
Reports that come back from second users. A report is published here only with the contributor's
explicit consent and after a second redaction pass (no product, plugin, skill or connector names
that identify an employer); otherwise only a one-line summary appears. Each one becomes issues and a release.
